...
// discordapp.com/oauth2/authorize?client_id=395826223437316097&scope=bot&permissions=0
//const LOGIN_TOKEN = 'Mzk1ODI2MjIzNDM3MzE2MDk3.DSYhbw._mhhLwIWqx1FAow0PthFTz2jbmY'; // test
const LOGIN_TOKEN = 'Mzg5OTU3MjY4OTc3NzQ1OTM1.DRDIBA.t71g-TSyLfE4OpK0Xzn8QGPZdbw';
const STATUS_OK = 'ok';
const MAX_RETRY = 3;
const MAX_CONCURRENCY = 10;
const WOWS_SERVER = 'https://api.worldofwarships.asia/wows/';
const APP_ID = '4b9e5858127fb558d366871013e54d1a';
const CLAN_TAG = 'LILY';
...
//    console.error(err);
//    return;
//    _client.clearInterval(_interval);
//    _client.destroy()
//        .then(_ => {
//            console.log('再起動します。');
//            _client = new Discord.Client();
//            _client.login(LOGIN_TOKEN);
//        })
//        .catch(err => console.log('再起動失敗' + err));
//});
...
let getClan = async function (tag) {
    ...
    let maxpage = 1;
    do {
        let res = await callApi(util.format('%sclans/list/?application_id=%s&search=%s&page_no=%d', WOWS_SERVER, APP_ID, tag, ++page));
...
let callApi = async function (uri) {
    let retry = 0;
    let opt = { uri: uri, json: true };
    let res = null;
    ...
    while (true) {
        ...
        if (res.status == STATUS_OK) {
            ...
            console.log('[%s] returns error.\n%s', uri, util.inspect(res.error, false, 5));
            if (++retry >= MAX_RETRY) {
                throw res.error;
            }
        }
...
// 指定チャンネルで操作を実行
function invokeChannel(type, name, permissions, func) {
    return Promise.map(_client.guilds.array(), guild => {
        ...
        // 書き込みには読み取り権限も必要
        if (!channel.permissionsFor(_client.user).has(permissions)) {
...
function dump(v) {
    return console.log(util.inspect(v, false, 5));
}
